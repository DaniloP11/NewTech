...
if($id == '' || $token ==''){
  ...
  if($token == $token_tmp){
    ...
    $sql = $con->prepare("SELECT count(id) FROM celulares WHERE id=? AND activo=1");
    $sql->execute([$id]);
    if($sql->fetchColumn() > 0){

      $sql = $con->prepare("SELECT nombre, descripcion, precio FROM celulares WHERE id=? AND activo=1 LIMIT 1");
      $sql->execute([$id]);
      ...
      $dir_images = 'images/celulares/' . $id . '/';
